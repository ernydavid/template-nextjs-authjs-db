...
import { deleteDepartmentById } from '@/actions/departments'
...
export function DeleteDepartmentDialog ({ id }: DeleteDialogProps) {
  const [error, setError] = useState<string | null>()
  const [isPending, startTransition] = useTransition()
  const [open, setOpen] = useState(false)

  const form = useForm<z.infer<typeof DeleteDepartmentSchema>>({
  ...
  const onSubmit = async (values: z.infer<typeof DeleteDepartmentSchema>) => {
    setError('')

    startTransition(() => {
      deleteDepartmentById(values.id).then((res) => {
        if (res.error) {
          setError(res.error)
        }
        if (res.success) {
