...
import z from 'zod'
import { deleteCountryById } from '@/actions/countries'
...
export function DeleteCountryDialog ({ id }: DeleteDialogProps) {
  const [error, setError] = useState<string | null>()
  const [isPending, startTransition] = useTransition()
  const [open, setOpen] = useState(false)

  const form = useForm<z.infer<typeof DeleteCountrySchema>>({
    resolver: zodResolver(DeleteCountrySchema),
    defaultValues: {
      id
    }
  })

  const onSubmit = async (values: z.infer<typeof DeleteCountrySchema>) => {
    setError('')

    startTransition(() => {
      deleteCountryById(values.id).then((res) => {
        if (res.error) {
          setError(res.error)
        }
        if (res.success) {
